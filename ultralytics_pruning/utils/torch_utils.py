...
from ultralytics_pruning.utils import (
    AUTOINSTALL,
    DEFAULT_CFG_DICT,
    DEFAULT_CFG_KEYS,
    LOGGER,
...
from ultralytics_pruning.utils.checks import check_version

# System environment variables
CUDA_DEVICES = True
TQDM_IS_TERMINAL = True
...
try:
    ...
    TORCH_CUDA_AMP_AVAILABLE = hasattr(torch.cuda.amp, "autocast")
except ImportError:
    TORCH_CUDA_AMP_AVAILABLE = False
    amp = None

# When cuda is unavailable, CUDA_DEVICES=False which avoids some errors
try:
...
# Check if cvxpy is installed, used for symmetric dist assignment in labeling.py
try:
    import cvxpy as cx
...
# Google AutoML Efficientnet import
try:
...
# Try to import memory_efficient_attention
try:
    import xformers
    import xformers.ops
...
# Persistent cache for method results
from ultralytics_pruning.utils import PERSISTENT_CACHE  # avoid circular import error

# Check for conflicts
CONFLICTS = []
if TIMM_AVAILABLE and EFFICIENTNET_PYTORCH_AVAILABLE:
    CONFLICTS.append(
        "EfficienetNet models from both timm and efficientnet_pytorch detected. Defaulting to timm but removing "
        "efficientnet_pytorch due to potential conflicts."
    )
    EFFICIENTNET_PYTORCH_AVAILABLE = False
    EffNet = None

# Flash attention not yet tested with YOLO
if FLASH_ATTENTION_AVAILABLE:
    FLASH_ATTENTION_AVAILABLE = False
    flash_attn_func = flash_attn_varlen_func = None

if CONFLICTS:
    LOGGER.warning(f"⚠️ Conflicts detected! {', '.join(CONFLICTS)}")
